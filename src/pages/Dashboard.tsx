...
const Dashboard = () => {
  ...
  const pastAppointments = useAppSelector((state) =>
    selectPastAppointments(state, 'user', user?.email)
  )

  const stats = useAppSelector((state) =>
    selectAppointmentStats(state, 'user', user?.email)
  )

  const chartData = useMemo(() => {
    const allAppointments = [...upcomingAppointments, ...pastAppointments]
    const monthlyData: {
      [key: string]: { agendamentos: number; receita: number }
    } = {}

    allAppointments.forEach((appointment) => {
      const monthKey = appointment.date.toLocaleString('pt-BR', {
        month: 'short'
      })
                ...
                <TabsContent value='upcoming' className='space-y-4'>
                  <div className='flex items-center justify-between mb-4'>
                    ...
                    <Button asChild size='sm'>
                      <a href='/agendamento'>Novo Agendamento</a>
                    </Button>
                  </div>
